...
pub fn main() {
	...
	let sample_code = fs::read_to_string("crates/does-it-throw/src/fixtures/sample.ts").expect("Something went wrong reading the file");
  ...
  for fun in result.functions_with_throws.into_iter() {
    ...
    println!("Function throws: {}, className {}", fun.function_or_method_name, fun.class_name.unwrap_or_else(|| "NOT_SET".to_string()));
    println!(
